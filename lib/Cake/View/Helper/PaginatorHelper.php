	...
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			//return $this->Html->tag($tag, $title, array_merge($options, compact('escape', 'class')));
						return  $this->Html->tag($tag, $this->link($title, array($title), array_merge($options,array('default'=>false), compact('escape', 'class'))), array('class' => 'prev disabled'));
 ...
 */
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, $this->link($i, array($params['page']), array_merge($options,array('default'=>false))), array('class' => 'active'));
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				...
				$out .= $this->Html->tag($tag, $this->link($i, array($i), array_merge($options,array('default'=>false))), array('class' => 'active'));
